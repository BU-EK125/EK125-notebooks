...
Unlike Google Colab, which has NumPy pre-installed, PyCharm requires you to install it manually — but only once per project. Think of it like downloading an app: you install it once, and then it's always available.

Follow these steps carefully before you try to run any code that uses NumPy.

### Step 1: Open the Python Packages tool window

With your project open in PyCharm, go to the menu bar and select:

**View → Tool Windows → Python Packages**

A panel will appear, usually at the bottom of the screen, with a search bar.

### Step 2: Search for NumPy

In the search bar, type `numpy`. You will see it appear in the results list.

### Step 3: Install

Click on `numpy` in the results list. On the right side of the panel, you will see a description and an **Install** button. Click **Install** and wait for it to finish — it usually takes 10–30 seconds.

> **Tip:** You'll know it worked when the Install button changes to **Installed** and no error messages appear at the bottom of the screen.

### Step 4: Verify the installation

Create a new `.py` file (or open your homework file) and run the following two lines:
...
If NumPy installed correctly, you will see a version number printed to the console, something like `1.26.4`. If you see an error instead, ask for help before continuing — everything else in this reading depends on NumPy being available.

### Troubleshooting

If you see `ModuleNotFoundError: No module named 'numpy'` after installing, the most common cause is that PyCharm is using a different Python interpreter than the one you installed NumPy into. To fix this:

1. Go to **File → Settings** (Windows/Linux) or **PyCharm → Settings** (Mac)
2. Navigate to **Project → Python Interpreter**
3. Make sure the interpreter shown matches the one you've been using for your other assignments (it should say something like `Python 3.x` with a path to your project's virtual environment)
4. If it looks wrong, click the gear icon and select the correct interpreter, then repeat the installation steps above

If you're still stuck, bring your laptop to the afternoon lab session and we'll sort it out together.
...
Once you have written code — especially numerical code — how do you know it is correct? The answer is **testing**: writing small checks that verify your program behaves as expected. This section introduces testing strategies that are particularly important when working with NumPy arrays.

### What is a test, and why write one?

A test is a small piece of code that checks that some part of your program produces the output you expect. There are several compelling reasons to write tests:

- **Correctness** — tests catch bugs early, before they affect results you're relying on
- **Regression guard** — if you later edit your code, tests confirm you haven't broken something that used to work
- **Documentation** — a well-written test shows exactly how a function is supposed to behave

In this course you will primarily write **unit tests**: tests that check one small piece of code — a single function or calculation — in isolation.

### `assert`: Python's simplest testing tool

The `assert` statement is the most direct way to write a test in Python. It checks whether a condition is `True`. If it is, nothing happens and the program continues. If it is `False`, Python raises an `AssertionError` and stops.
...
You can include an optional message after a comma to make failures easier to understand:

```python
>>> assert x > 0, "x should be positive"
```

If the condition is `False`, the message appears in the error output:

```
AssertionError: x should be positive
```

`assert` is a **statement**, not a function — notice there are no parentheses required around the condition. It works with any expression that evaluates to `True` or `False`.

### The array `assert` gotcha

You might expect that testing whether two arrays are equal is as simple as writing `assert a == b`. However, this does **not** work as intended with NumPy arrays:
...
The problem is that `a == b` returns an **array** of boolean values, not a single `True` or `False`. Python does not know how to evaluate an array as a single condition, so it raises a `ValueError`.
...
The correct pattern is to use the `.all()` method, which returns `True` only if every element in the boolean array is `True`:

```python
>>> assert (a == b).all(), "Arrays must match exactly"
```

### Floating point error

Even when two calculations *should* give the same answer, they sometimes do not — due to how computers store decimal numbers in binary.

Floating point numbers are stored using a fixed number of bits. Many decimal values (such as 0.1) cannot be represented exactly in binary, which leads to tiny rounding errors. These errors can accumulate across operations, and because they are so small they are easy to miss.
...
The difference here is less than 6 × 10⁻¹⁷ — completely invisible at normal precision, but enough to make `==` return `False`. The same kind of problem can arise with NumPy arrays when two arrays are computed via different routes.

The solution is to compare with a **tolerance** using `np.allclose()` instead of `==`. This function returns `True` if all corresponding elements are within a small threshold of each other:

```python
>>> np.allclose(x, y)
True
>>> assert np.allclose(x, y), "Values should be approximately equal"
```

For scalar comparisons, the standard library also provides `math.isclose(x, y)`.

### NumPy's specialized testing tools

For more detailed and informative tests, NumPy provides a set of dedicated functions in the `np.testing` module. These are preferable to raw `assert` for numerical work because they automatically handle shape checking, dtype, and element-wise comparison — and they produce clear error messages that tell you exactly *where* arrays differ, which is invaluable when debugging.

The most commonly used functions are:

**`np.testing.assert_array_equal(a, b)`** checks that two arrays have exactly the same shape and every element is identical. Use this for integer arrays or any case where exact equality is expected:
...
**`np.testing.assert_allclose(a, b, atol=...)`** checks that two arrays are close within a specified tolerance. This is the right tool whenever your results involve floating point arithmetic. The `atol` parameter sets the *absolute tolerance* — the maximum allowed difference between any two corresponding elements:
...
Here is a more complete example showing how these tools can be combined to test a NumPy computation:

```python
>>> a = array([1, 2, 3])
>>> norm = a / a.sum()
>>> expected = array([1/6, 2/6, 3/6])
>>> np.testing.assert_allclose(norm.sum(), 1.0)
>>> np.testing.assert_allclose(norm, expected)
```

Other functions in `np.testing` include `assert_equal` (for arrays, scalars, and nested structures), `assert_array_less`, and `assert_raises`. You can read about them in the [NumPy testing documentation](https://numpy.org/doc/stable/reference/routines.testing.html).

### What to test — and what not to

The goal of a test is to confirm behavior, not to restate the code. Some guidelines to keep in mind:

- Always test the **normal case**: does the function give the right answer for typical input?
- Test **edge cases**: what happens with an empty array? A single-element array? An array of all zeros?
- For floating-point results, **always use tolerance-based comparison** (`np.allclose` or `assert_allclose`), never `==`.
- When testing code that uses random numbers, **fix the seed** so that results are reproducible:

```python
>>> np.random.seed(0)
>>> arr = np.random.rand(5)
>>> np.testing.assert_allclose(arr[0], 0.5488135, atol=1e-6)
```

One important pitfall is writing **trivial asserts** — tests that are always `True` regardless of whether the code is correct. These give you a false sense of security. Always ask yourself: would this test fail if my code were wrong?
