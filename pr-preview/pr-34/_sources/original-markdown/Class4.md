...
In Python, everything is an **object**: numbers, strings, lists, dictionaries. 

**Object Oriented Programming** is a very high-level idea and deep topic (don’t worry about this yet).  The thing to note for now, is that in Python, values like numbers, strings, and lists are all treated in a uniform way \- as objects.  This means they can carry both data (the value itself) and built-in operations that know how to work with that data.  These operations are called **methods**.  

**Objects** are the combination of data storage and associated methods.  And what matters most, for now, is that different kinds of values (e.g. strings, lists, numerics) come with different built-in operations, which we access using dot notation.  

You can contrast this to non-object oriented programming \- data storage without any associated methods.  In such cases all the operations that you can perform on the data storage is defined and implemented completely independently of that storage.  

We've seen lots of objects and methods already, so some of the following may look familiar. But we wanted to formally point out the notion of **objects** and **methods**, which are important concepts as we move forward\!
...
right\_aligned \= f"{num:8d}"   \# right-aligned (default)

left\_aligned  \= f"{num:\<8d}"  \# left-aligned

centered      \= f"{num:^8d}"  \# centered

print("Integer alignment examples:")

print(f"Right-aligned: '{right\_aligned}'")

print(f"Left-aligned:  '{left\_aligned}'")
...
formatted\_pi \= f"{pi:8.3f}"   \# width 8, 3 decimal places

print("\\nFloating-point example:")

print(f"Formatted pi: '{formatted\_pi}'")
